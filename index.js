...
app.post("/api/boost/energy", async (req, res) => {
  try {
    ...
    // If the user chose the free "sponsor" path (shown as "Cancel" in the dialog),
    // return the mission code + URL so the client can open the smart-link.
    let sponsor_url = null;
    if (method !== "points") {
      const m = await pool.query(
        "SELECT url FROM public.missions WHERE code = $1 LIMIT 1",
        ["sp_emergency_energy"]
      );
      sponsor_url = m.rows?.[0]?.url || null;
    }
    ...
    return res.json({
      ...state,
      ok: true,
      sponsor_mission_code: method !== "points" ? "sp_emergency_energy" : undefined,
      sponsor_url: method !== "points" ? sponsor_url : undefined,
      message:
        method === "points"
...
app.post("/api/boost/double", async (req, res) => {
  try {
    ...
    // If the user chose the free "sponsor action" path, also return the
    // sponsor mission URL so the client can actually open it.
    let sponsor_url = null;
    if (method === "sponsor") {
      const m = await pool.query(
        `SELECT url FROM public.missions WHERE code = $1 LIMIT 1;`,
    ...
    return res.json({
      ...
      ok: true,
      sponsor_url,
      sponsor_mission_code: method === "sponsor" ? "sp_double_points" : undefined,
      message:
        method === "points"
