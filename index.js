...
app.post("/api/shop/starter-offer", async (req, res) => {
  try {
    ...
    const telegramIdRaw = String(user.telegram_id);

    const starterSku = process.env.STARTER_PACK_SKU || "STARTER_1";

    // Ensure they are a normal player
    const isSystem = !!user.is_system_user;
    if (!user || isSystem) {
      return res.json({
        success: true,
        available: false,
    ...
    const alreadyPurchasedFlag = user && user.starter_offer_purchased === true;
...
// Telegram WebApp initData verification (mandatory for production security).
// We validate the signature using: secret_key = HMAC_SHA256("WebAppData", BOT_TOKEN)
// and compare against the provided `hash`.
// Ref: https://core.telegram.org/bots/webapps#validating-data-received-via-the-web-app
function verifyTelegramInitData(initDataRaw, { maxAgeSeconds = 86400 } = {}) {
  if (!initDataRaw) return { ok: false, error: "MISSING_INITDATA" };
  ...
  const a = Buffer.from(String(computed), "utf8");
...
async function ensureSchema(client) {
  ...
  // Balance ledger (append-only) – source of truth for all balance changes.
  await client.query(`
    CREATE TABLE IF NOT EXISTS public.user_balance_ledger (
  ...
  await client.query(`
    CREATE INDEX IF NOT EXISTS idx_ubl_user_created ON public.user_balance_ledger (user_id, created_at DESC);
  `);
...
// Get or create a user from Telegram initData.
// NOTE: In production we do NOT allow telegram_id fallbacks. This is required to
// match Hamster/Notcoin-class security assumptions.
async function getOrCreateUserFromInitData(req) {
  const initDataRaw = getInitDataFromReq(req);

  const v = verifyTelegramInitData(initDataRaw, { maxAgeSeconds: 86400 });
  if (!v.ok) {
    const err = new Error(v.error || "INVALID_INITDATA");
    err.code = v.error || "INVALID_INITDATA";
    throw err;
  }
  ...
  if (!telegramUserId) {
    ...
    err.code = "MISSING_TELEGRAM_USER";
    throw err;
  }
...
app.get("/api/state-debug", async (req, res) => {
  try {
    ...
    const telegramId = Number(req.query.telegram_id || 0);
    if (!telegramId) {
      res.write("ERROR: BAD_TELEGRAM_ID\n");
      return res.end();
    }
    ...
    if (!user) {
      res.write("ERROR: USER_NOT_FOUND\n");
      return res.end();
    }
