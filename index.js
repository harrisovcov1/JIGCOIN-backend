...
async function applyEnergyRegen(user, opts = {}) {
  const ignoreRecentTap = !!opts.ignoreRecentTap;

  // If the user is actively tapping, do NOT regen on the server.
  // This prevents the classic "energy doesn't go down" bug when regen and spend happen in the same loop.
  if (!ignoreRecentTap && user.last_tap_ts) {
    const lastTap = new Date(user.last_tap_ts);
    if (!isNaN(lastTap)) {
      const msSinceTap = Date.now() - lastTap.getTime();
      if (msSinceTap >= 0 && msSinceTap < 1500) return user;
    }
  }
  ...
  // If no full "step" elapsed, do not touch DB timestamps (keeps remainder for smooth regen).
  const consumedMs = Math.max(0, originalDiffMs - diffMs);
  if (consumedMs <= 0) return user;

  const newLast = new Date(last.getTime() + consumedMs);
  ...
  await pool.query(
    ...
    WHERE id = $3
    `,
    [energy, newLast.toISOString(), user.id]
  );
