...
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    if (missionRes.rowCount === 0) return res.status(404).json({ ok: false, error: "MISSION_NOT_FOUND" });
    ...
    if (umRes.rowCount === 0 || !umRes.rows[0].started_at) {
    ...
    if (um.claimed_at) {
      ...
      if (readyAt > now) {
        ...
        return res.json({ ...(await buildClientState(user)), ok: false, error: "MISSION_COOLDOWN", cooldown_remaining_seconds: remaining });
      }
    }
