...
// Ensure schema runs only once per process (Render can send many concurrent requests)
...
async function ensureSchema(client) {
  ...
  // Make sure "code" is actually unique even if the table existed before this file
  await client.query(`
  ...
  // Older DB migrations / constraints may use cooldown_hours instead of seconds
  await client.query(`
    ALTER TABLE public.missions
  ...
  // Sponsor profitability fields used by your Supabase constraints/migrations
  await client.query(`
    ALTER TABLE public.missions
  ...
  // Backfill to satisfy already-applied DB constraints (safe no-op if columns missing)
  // 1) If cooldown_hours exists but is NULL, derive it from cooldown_seconds.
  // 2) Ensure sponsor_cpc is at least payout_amount so "sponsor must be profitable" checks pass.
  await client.query(`
    UPDATE public.missions
      SET cooldown_hours = CASE
        WHEN cooldown_hours IS NULL AND COALESCE(cooldown_seconds, 0) > 0
          THEN CEIL(COALESCE(cooldown_seconds, 0) / 3600.0)
        WHEN cooldown_hours IS NULL
          THEN 0
        ELSE cooldown_hours
      END
  ...
  await client.query(`
    UPDATE public.missions
      SET sponsor_cpc = GREATEST(COALESCE(sponsor_cpc, 0), COALESCE(payout_amount, 0)),
          profit_per_claim = GREATEST(COALESCE(sponsor_cpc, 0), COALESCE(payout_amount, 0)) - COALESCE(payout_amount, 0)
    WHERE kind = 'sponsor';
  `);

  // Seed starter sponsor missions (safe upsert) — replace URLs later with real partner links
  // IMPORTANT: we set sponsor_cpc >= payout_amount so Supabase "profitability" constraints do not fail.
  await client.query(`
    INSERT INTO public.missions (code, title, description, payout_type, payout_amount, url, kind, cooldown_seconds, cooldown_hours, min_seconds_to_claim, sponsor_cpc, profit_per_claim, is_active)
    VALUES
      ('sponsor_1', 'Visit partner site', 'Open the partner page and spend a moment on it.', 'points', 500, 'https://example.com', 'sponsor', 86400, 24, 90, 500, 0, FALSE),
      ('sponsor_2', 'Join partner Telegram', 'Join the partner channel and stay for a moment.', 'points', 750, 'https://example.com', 'sponsor', 86400, 24, 120, 750, 0, FALSE),
      ('sponsor_3', 'Follow partner on X', 'Follow the partner profile on X and return.', 'points', 600, 'https://example.com', 'sponsor', 86400, 24, 60, 600, 0, FALSE),
      ('sponsor_4', 'Install partner mini-app', 'Open or install the partner mini-app.', 'points', 1000, 'https://example.com', 'sponsor', 86400, 24, 120, 1000, 0, FALSE),
      ('sponsor_5', 'Daily sponsor check', 'Complete today''s sponsor action and claim.', 'points', 400, 'https://example.com', 'sponsor', 86400, 24, 90, 400, 0, FALSE)
    ON CONFLICT (code) DO UPDATE SET
      ...
      sponsor_cpc = EXCLUDED.sponsor_cpc,
      profit_per_claim = EXCLUDED.profit_per_claim,
