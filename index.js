...
async function getOrCreateUserFromInitData(req) {
  ...
  try {
    if (AUTO_MIGRATE) 
    // Referral: capture payload from Telegram /startapp or direct URL param passed by the frontend.
    const startParamRaw = (req.get('x-start-param') || '').trim();
    let referredBy = '';
    if (startParamRaw) {
      referredBy = startParamRaw.startsWith('ref_') ? startParamRaw.slice(4) : startParamRaw;
    }

    // Stable per-user invite code (base36 Telegram ID)
    const inviteCode = `u${telegramUserId.toString(36)}`;
await ensureSchema(client);
    ...
    const upsertRes = await client.query(
      ...
      ALTER TABLE public.users ADD COLUMN IF NOT EXISTS referrals INTEGER NOT NULL DEFAULT 0;
      ALTER TABLE public.users ADD COLUMN IF NOT EXISTS referred_by TEXT;
      ALTER TABLE public.users ADD COLUMN IF NOT EXISTS today_farmed NUMERIC NOT NULL DEFAULT 0;
            ) VALUES (
              $1,$2,$3,$4,$5,
              0, 100, 100, 0,
              0,
            ...
            ON CONFLICT (telegram_id) DO UPDATE SET
              ...
              referred_by = CASE
                WHEN (public.users.referred_by IS NULL OR public.users.referred_by = '') AND EXCLUDED.referred_by IS NOT NULL
                  THEN EXCLUDED.referred_by
                ELSE public.users.referred_by
              END,
...
await client.query(`INSERT INTO public.user_devices (user_id, fingerprint) VALUES ($1,$2)
      ...
      [telegramUserId, username, firstName, lastName, languageCode, referredBy, inviteCode]
    ]);
...
async function maybeActivateReferral(invitedUser, trigger = 'tap') {
  ...
  try {
    ...
    const inviterRes = await pool.query(
      `SELECT id, COALESCE(referrals_count, referrals) AS referrals_count FROM public.users WHERE telegram_id=$1 LIMIT 1;`,
      [inviterTelegramId]
    );
...
async function buildClientState(user) {
  const inviteLink = `https://t.me/${BOT_USERNAME}?start=ref_${user.invite_code || user.telegram_id}`;
  ...
  return {
    ...
    referrals_count: Number(user.referrals ?? user.referrals_count ?? 0),
...
app.get("/api/admin/export/users.csv", async (req, res) => {
  try {
    ...
    const q = await pool.query(`SELECT id, telegram_id, username, balance, COALESCE(referrals_count, referrals) AS referrals_count, vip_until, streak_count, last_checkin_date, created_at FROM public.users ORDER BY id DESC LIMIT 50000`);
...
app.post("/api/referral/status", async (req, res) => {
  try {
    const user = await getOrCreateUserFromInitData(req);
    const tierInfo = getReferralTier(user.referrals_count ?? user.referrals ?? 0);
    const nextTargets = [1, 5, 20, 50, 200];
    const next = nextTargets.find((t) => Number(user.referrals_count ?? user.referrals ?? 0) < t) || null;
    ...
    return res.json({
      ...
      referral: {
        ...
        next_target: next,
        next_remaining: next ? Math.max(0, next - Number(user.referrals_count ?? user.referrals ?? 0)) : 0,
      },
    });
  } catch (err) {
...
bot.start(async (ctx) => {
  try {
    ...
    // Ensure schema exists (so users table is available)
    await ensureSchema();

    // Create/update the user record (without trusting referral here)
    await pool.query(
      `INSERT INTO public.users (telegram_id, username, first_name, last_name, language_code, balance, energy, max_energy, today_farmed, referrals, invite_code)
       VALUES ($1,$2,$3,$4,$5,0,100,100,0,0,$6)
       ON CONFLICT (telegram_id) DO UPDATE SET
         username = EXCLUDED.username,
         first_name = EXCLUDED.first_name,
         last_name = EXCLUDED.last_name,
      ...
      [
        ...
        `u${telegramId.toString(36)}`
      ]
    );

    // Referral payload format we generate: "ref_<invite_code>"
    // Also accept "ref_<telegram_id>" for backwards compatibility.
    let refToken = '';
    if (startPayload) {
      refToken = startPayload.startsWith('ref_') ? startPayload.slice(4) : startPayload;
    }

    if (refToken) {
      // Find inviter by invite_code first; fallback to telegram_id if numeric
      const inviterRes = await pool.query(
        `SELECT telegram_id, invite_code
         FROM public.users
         WHERE invite_code = $1
            OR (telegram_id = $2 AND $2 IS NOT NULL)
         LIMIT 1`,
        [refToken, /^\d+$/.test(refToken) ? BigInt(refToken) : null]
      );

      const inviter = inviterRes.rows[0];

      // Only set referral if inviter exists and isn't the same user
      if (inviter && inviter.telegram_id !== telegramId) {
        // Set referred_by ONCE (first-touch)
        const setRef = await pool.query(
          `UPDATE public.users
           SET referred_by = $1
           WHERE telegram_id = $2
             AND (referred_by IS NULL OR referred_by = '')
           RETURNING referred_by`,
          [inviter.invite_code || String(inviter.telegram_id), telegramId]
        );

        // If we actually set it now, increment inviter's referrals counter
        if (setRef.rowCount > 0) {
          await pool.query(
            `UPDATE public.users
    ...
    const appUrl = process.env.WEBAPP_URL || 'https://your-app-url.example.com';
    await ctx.reply(
      `Welcome to JIGCOIN 🚀\n\nTap to earn, invite friends, and climb the leaderboard.\n\nOpen the app: ${appUrl}`
    );
  } catch (e) {
