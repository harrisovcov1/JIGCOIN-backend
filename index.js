...
async function maybeAwardEarlyAccessStarterBonus(client, user) {
  ...
  if (!cap || !bonus) return { awarded: false, reason: 'disabled' };

  // Respect existing early access window end if you use it elsewhere
  const endTs = process.env.EARLY_ACCESS_END_TS ? Number(process.env.EARLY_ACCESS_END_TS) : null;
  if (endTs && Date.now() > endTs) return { awarded: false, reason: 'ended' };
  ...
  if (already.rowCount > 0) return { awarded: false, reason: 'already_claimed' };

  // Serialize award decisions
  // Any constant 32-bit int works. Keep stable across deploys.
  ...
  if (already2.rowCount > 0) return { awarded: false, reason: 'already_claimed' };

  // Determine deterministic rank of this user
  const rankRes = await client.query(
    `SELECT COUNT(*)::int AS r
       FROM public.users u2
      WHERE (u2.created_at < $1)
         OR (u2.created_at = $1 AND u2.id <= $2)`,
    [user.created_at, user.id]
  );
  const rank = rankRes.rows?.[0]?.r ?? null;
  if (!rank || rank > cap) return { awarded: false, reason: 'not_in_first_cap', rank };

  // Global cap based on awards issued (prevents >cap even with deletes/tests)
  const issuedRes = await client.query(
    `SELECT COUNT(*)::int AS c FROM public.event_log WHERE event='early_access_starter_bonus'`
  );
  const issued = issuedRes.rows?.[0]?.c ?? 0;
  if (issued >= cap) return { awarded: false, reason: 'cap_reached', rank, issued };

  // Award
  const beforeBal = Number(user.balance || 0);
  await client.query(`UPDATE public.users SET balance = balance + $1 WHERE id=$2`, [bonus, user.id]);
  await client.query(
    `INSERT INTO public.event_log(user_id, event, meta)
     VALUES ($1, 'early_access_starter_bonus', $2::jsonb)`,
    [
      user.id,
      JSON.stringify({ amount: bonus, before: beforeBal, after: beforeBal + bonus, rank, issued_before: issued })
    ]
  );
  return { awarded: true, bonus, rank, issued_before: issued };
}
