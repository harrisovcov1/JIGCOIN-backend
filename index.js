...
async function ensureSchema(client) {
  ...
  await client.query(`
    INSERT INTO public.missions (code, title, description, payout_type, payout_amount, url, kind, cooldown_seconds, is_active)
    VALUES
      ('sponsor_1', 'Visit partner site', 'Open the partner page and come back to claim.', 'points', 500, 'https://example.com', 'sponsor', 86400, TRUE),
      ('sponsor_2', 'Watch sponsor video', 'Complete a quick sponsor action, then claim.', 'points', 750, 'https://example.com', 'sponsor', 86400, TRUE),
      ('sponsor_3', 'Install partner app', 'Install or open the partner app and return to claim.', 'points', 1000, 'https://example.com', 'sponsor', 86400, TRUE)
...
function missionCooldownRemainingSeconds(mission, userMissionRow) {
...
app.post("/api/mission/list", async (req, res) => {
  try {
    ...
    const missionsRes = await pool.query(
      `
      SELECT id, code, title, description, payout_type, payout_amount, url, kind, COALESCE(cooldown_seconds,0) AS cooldown_seconds
    ...
    const missions = missionsRes.rows.map((m) => {
      ...
      // For repeatable missions (e.g. sponsor), expose cooldown and reset status when cooldown passes
      let cooldown_remaining_seconds = 0;
      let status = um ? um.status : "not_started";
      let reward_applied = um ? um.reward_applied : false;

      if (cooldownSeconds > 0 && completedAt) {
        const availableAtMs = completedAt + cooldownSeconds * 1000;
        const remainingMs = Math.max(0, availableAtMs - nowMs);
        cooldown_remaining_seconds = Math.floor(remainingMs / 1000);

        if (cooldown_remaining_seconds > 0) {
          status = "cooldown";
          // while in cooldown, treat as completed so UI can't claim
          reward_applied = true;
        } else {
          // cooldown passed: allow start/claim again
          status = "not_started";
...
app.post("/api/mission/start", async (req, res) => {
  try {
    ...
    const remainingStart = missionCooldownRemainingSeconds(mission, existingUM.rows[0]);
    if (remainingStart > 0) {
...
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    // Enforce cooldown for repeatable missions (e.g. sponsor quests)
    const existingUM = await pool.query(
      `SELECT id, completed_at, reward_applied FROM public.user_missions WHERE user_id = $1 AND mission_id = $2 LIMIT 1;`,
      [user.id, mission.id]
    );
    const remaining = missionCooldownRemainingSeconds(mission, existingUM.rows[0]);
    if (remaining > 0) {
      return res.json({
        ok: false,
