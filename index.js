...
async function getOrCreateUserFromInitData(req) {
  ...
  try {
    ...
    if (inviterId && inviterId && inviterId !== telegramUserId) {
      // Only allow a single inviter per invited user (first referral wins).
      // Also prevent simple 2-node and small-chain loops by walking inviter->referred_by chain.
      let loopDetected = false;
      try {
        const inviterRow = await client.query(
          `SELECT id, referred_by FROM public.users WHERE telegram_id = $1 LIMIT 1;`,
          [inviterId]
        );
        let cur = inviterRow.rows[0]?.referred_by || null;
        let hops = 0;
        while (cur && hops < 10) {
          if (Number(cur) === Number(user.id)) { loopDetected = true; break; }
          const r = await client.query(`SELECT referred_by FROM public.users WHERE id = $1 LIMIT 1;`, [cur]);
          cur = r.rows[0]?.referred_by || null;
          hops++;
        }
      } catch (e) {}

      if (!loopDetected) {
        // Ensure referral row exists ONLY if invited_id has no referrer yet.
        const insRef = await client.query(
          `INSERT INTO public.referrals (inviter_id, invited_id)
           SELECT $1, $2
           WHERE NOT EXISTS (SELECT 1 FROM public.referrals WHERE invited_id = $2)
           ON CONFLICT (inviter_id, invited_id) DO NOTHING
           RETURNING 1;`,
          [inviterId, telegramUserId]
        );

        // Only act on the very first successful insert (first inviter wins).
        if (insRef.rowCount > 0) {
          ...
          if (inviterRes.rowCount > 0) {
            ...
            // Persist referrer on invited user (first writer wins).
            try {
              await client.query(
                `
                UPDATE public.users
            ...
            // Reward immediately ONLY when strict activation is disabled.
