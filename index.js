...
// ---- DEBUG: log tap requests (temporary) ----
app.use((req, res, next) => {
  const start = Date.now();
  res.on('finish', () => {
    if (req.path === '/api/tap' || req.path === '/api/tapPacket') {
      console.log('[REQ]', req.method, req.path, 'status:', res.statusCode, 'ms:', Date.now() - start);
    }
  });
  next();
});
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    console.log('[TAP SAVE]', {
      ...
      today_farmed: updatedUser.today_farmed,
      taps_today: updatedUser.taps_today,
