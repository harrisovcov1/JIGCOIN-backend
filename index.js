...
// Early Access starter bonus (first N unique awardees get bonus once)
const EARLY_STARTER_BONUS = Number(process.env.EARLY_STARTER_BONUS || 5000);
const EARLY_STARTER_CAP = Number(process.env.EARLY_STARTER_CAP || 1000);
...
app.get("/api/stats", async (_req, res) => {
  try {
    ...
    const limit = 1000;
    const awardRes = await pool.query(`SELECT COUNT(*)::int AS c FROM public.early_access_starter_bonus;`);
    const awardedCount = Number(awardRes.rows?.[0]?.c || 0);
    const limit = EARLY_STARTER_CAP;
    const remaining = Math.max(0, limit - awardedCount);
    const now = Date.now();
    ...
    return res.json({
      ...
      early_bonus_awarded: awardedCount,
      early_bonus_active: awardedCount < limit && timeOk,
...
-- Track Early Access starter bonus awards (one-time per user, capped)
CREATE TABLE IF NOT EXISTS public.early_access_starter_bonus (
...
async function getOrCreateUserFromInitData(req) {
  ...
  try {
    ...
    // Early Access starter bonus: credit once for first N eligible users
    await maybeAwardEarlyStarterBonus(client, telegramUserId);
...
async function maybeAwardEarlyStarterBonus(client, telegramUserId) {
  // Only during Early Access time window (if set)
  const now = Date.now();
  const endsAt = EARLY_ACCESS_END_TS ? Number(EARLY_ACCESS_END_TS) : null;
  const timeOk = endsAt ? now < endsAt : true;
  if (!timeOk) return { awarded: false, reason: "ended" };

  // Concurrency-safe cap: one-time awards for first EARLY_STARTER_CAP unique users
  await client.query("BEGIN");
  try {
    // Lock a single global key for the duration of this transaction
    await client.query("SELECT pg_advisory_xact_lock(987654321);");

    const existsRes = await client.query(
      "SELECT 1 FROM public.early_access_starter_bonus WHERE telegram_id = $1 LIMIT 1;",
      [telegramUserId]
    );
    ...
    const countRes = await client.query(
      "SELECT COUNT(*)::int AS c FROM public.early_access_starter_bonus;"
    );
    const awardedCount = Number(countRes.rows?.[0]?.c || 0);
    if (awardedCount >= EARLY_STARTER_CAP) {
      await client.query("COMMIT");
      return { awarded: false, reason: "cap_reached" };
    }

    // Mark award first (idempotent with PK)
    await client.query(
      "INSERT INTO public.early_access_starter_bonus (telegram_id) VALUES ($1);",
      [telegramUserId]
    );

    // Credit bonus
    await client.query(
      "UPDATE public.users SET balance = COALESCE(balance, 0) + $2 WHERE telegram_id = $1;",
      [telegramUserId, EARLY_STARTER_BONUS]
    );
