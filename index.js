      ...
      if (telegramId) {
        ...
        await pool.query(
          ...
          [telegramId, durationDays]
	        ...
	        // 4️⃣ + 5️⃣ Optional but recommended: store purchase + ledger entries
	        // (gives admin audit trail, revenue tracking, purchase history, refund reversals)
	        try {
	          // Fetch internal bigint user id if present
	          const userRes = await pool.query(
	            `SELECT id FROM public.users WHERE telegram_id = $1 LIMIT 1`,
	            [telegramId]
	          );
	          const userIdBigint = userRes.rows[0] ? Number(userRes.rows[0].id) : null;

	          const stripeSessionId = session.id;
	          const amount = Number(session.amount_total || 0); // Stripe gives smallest currency unit (e.g. pence)
	          const currency = (session.currency || 'gbp').toLowerCase();

	          // Idempotency for purchases insert
	          const existingPurchase = await pool.query(
	            `SELECT id FROM public.purchases WHERE stripe_session_id = $1 LIMIT 1`,
	            [stripeSessionId]
	          );
	          let purchaseId = existingPurchase.rows[0] ? existingPurchase.rows[0].id : null;
	          ...
	          if (!purchaseId) {
	            ...
	            const ins = await pool.query(
	              `INSERT INTO public.purchases
	                 (telegram_id, user_id, pack_id, amount, currency, stripe_session_id, status, meta)
	               VALUES ($1, $2, $3, $4, $5, $6, $7, $8::jsonb)
	               RETURNING id`,
	              [telegramId, userIdBigint, null, amount, currency, stripeSessionId, 'paid', JSON.stringify(meta)]
	            );
	            purchaseId = ins.rows[0] ? ins.rows[0].id : null;
	          }

	          // Ledger entry (best-effort; schema may allow NULL user_id uuid)
	          const ledgerMeta = {
	            purchase_id: purchaseId,
	            user_id_bigint: userIdBigint,
	            kind: 'vip',
	            duration_days: durationDays,
	          ...
	          await pool.query(
	            `INSERT INTO public.purchase_ledger
	               (telegram_id, user_id, type, amount, currency, source, reference, metadata)
	             VALUES ($1, $2, $3, $4, $5, $6, $7, $8::jsonb)`,
	            [
	              telegramId,
	              null, // purchase_ledger.user_id is uuid in your schema inventory; keep NULL and store bigint in metadata
	              'purchase',
	              amount,
	              currency,
	              'stripe',
	          ...
	          // Don't fail the webhook for audit logging issues (the credit already happened)
	          console.error('Purchase logging failed:', e);
	        }
      }
      handled = true;
    }
