...
async function ensureSchema(client) {
  ...
  await client.query(`
    ...
    -- Some parts of the backend use energy_max (legacy/new naming). Keep it in sync with max_energy.
  ...
  // Keep users.energy_max in sync with users.max_energy (some code / legacy schema uses energy_max).
  await client.query(`
    CREATE OR REPLACE FUNCTION public.sync_energy_max() RETURNS trigger AS $$
    BEGIN
      -- If max_energy is set/changed, mirror it to energy_max.
      IF NEW.max_energy IS NOT NULL THEN
        NEW.energy_max := NEW.max_energy;
      ELSIF NEW.energy_max IS NULL THEN
        NEW.energy_max := COALESCE(NEW.max_energy, 50);
      END IF;
      RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
    ...
    DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sync_energy_max') THEN
        CREATE TRIGGER trg_sync_energy_max
        BEFORE INSERT OR UPDATE OF max_energy
        ON public.users
        FOR EACH ROW
        EXECUTE FUNCTION public.sync_energy_max();
      END IF;
    END $$;

    UPDATE public.users
    SET energy_max = max_energy
    WHERE energy_max IS NULL AND max_energy IS NOT NULL;
  `);
  ...
  await client.query(`
    ...
    -- Code paths use reward_amount; keep both.
  ...
  // Best-effort backfill so reward_amount exists when code expects it.
  await client.query(`
    UPDATE public.missions
    SET reward_amount = COALESCE(reward_amount, payout_amount, reward_value)
...
app.post("/api/mission/complete", async (req, res) => {
  try {
    ...
    if (!um.reward_applied) {
      ...
      const rValueRaw = mission.reward_amount ?? mission.reward_value ?? mission.payout_amount ?? 0;
      ...
      } else if (rTypeRaw === "mission") {
        // Sponsor missions: billing/ledger updates are handled by sponsorChargeIfNeeded(...).
        // If a numeric reward is configured, treat it as points.
        if (rValue > 0) {
          user = await applyGenericReward(user, "points", rValue, pool);
        }
        rewardApplied = true;
