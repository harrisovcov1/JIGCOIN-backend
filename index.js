...
function missionCooldownRemainingSeconds(mission, user, userMissionRow) {
  // Global sponsor cooldown (one sponsor quest per 24h total)
  if ((mission.kind || "") === "sponsor") {
    const last = user && user.last_sponsor_ts ? Date.parse(user.last_sponsor_ts) : NaN;
    ...
    const missions = missionsRes.rows.map((m) => {
      ...
      if (true) {
        ...
        if (cooldown_remaining_seconds > 0) {
          ...
          // if we previously showed cooldown, reset to allow a new cycle
          if (status === "cooldown") status = "not_started";
          if (status === "completed") status = "not_started";
          if (status === "not_started") reward_applied = false;
        }
      }
