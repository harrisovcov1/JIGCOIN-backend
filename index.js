...
const COINBASE_API_VERSION = process.env.COINBASE_API_VERSION || "2018-03-22";
...
async function createCoinbaseCharge({ name, description, amount, currency = "GBP", metadata = {}, redirect_url, cancel_url }) {
  ...
  if (!resp.ok) {
    const msg = json?.error?.message || json?.message || `Coinbase API error (${resp.status})`;
    throw new Error(msg);
  }
  return json?.data;
}

function verifyCoinbaseSignature(rawBody, signature) {
  if (!COINBASE_WEBHOOK_SECRET) return false;
  if (!rawBody || !signature) return false;
  const computed = crypto.createHmac("sha256", COINBASE_WEBHOOK_SECRET).update(rawBody).digest("hex");
  try {
...
// Optional but recommended: persist purchases + ledger for audit/revenue/user history
async function recordPurchaseAndLedger({
  publicUserId,
  telegramId,
  kind,
  provider,
  ...
  const amtMinor = Number(amountMinor || 0);
  const cur = (currency || 'usd').toString().toLowerCase();
  const meta = metadata || {};

  // 1) public.purchases (simple purchase history)
  try {
    await pool.query(
      `INSERT INTO public.purchases (user_id, telegram_id, amount_usd, currency, provider, provider_tx, status, meta)
       VALUES ($1, $2, $3, $4, $5, $6, 'paid', $7)
      `,
      [
        publicUserId ?? null,
        telegramId ?? null,
        // Store a reasonable numeric value (USD if you price in USD; otherwise it's still useful as "amount")
        amtMinor / 100,
        cur,
        provider || 'stripe',
        providerTx || stripePaymentIntentId || stripeCheckoutSessionId || null,
        meta,
      ]
    );
  } catch (e) {
    // If table doesn't exist yet or constraints differ, don't break the webhook.
    if (e && e.code !== '23505') {
      console.warn('purchases insert skipped:', e.message || e);
    }
  }

  // 2) public.purchase_ledger (auditable ledger; supports refunds/reversals)
  try {
    await pool.query(
      `INSERT INTO public.purchase_ledger (
         ...
         pack_sku, coins_granted,
         status, metadata
       ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'paid',$10)
      `,
      [
        // Some schemas use auth UUID here; if you don't have it, NULL is OK in most setups.
        null,
        kind || 'purchase',
        provider || 'stripe',
    ...
    if (e && e.code !== '23505') {
      console.warn('purchase_ledger insert skipped:', e.message || e);
    }
  }
}
...
/**
 * Coinbase Commerce: create a charge for a coin pack
 * Frontend should POST { telegram_id, pack_id } and then redirect user to hosted_url
 */
app.post("/api/coinbase/create-charge", async (req, res) => {
  try {
    const { telegram_id, pack_id } = req.body || {};
    if (!telegram_id || !pack_id) {
      return res.status(400).json({ error: "telegram_id and pack_id are required" });
    }

    // Load pack from Supabase
    ...
    // Defensive mapping in case column names differ
    const price =
      pack.price_gbp ??
      pack.gbp_price ??
      pack.price ??
    ...
    const redirectUrl = webAppUrl ? `${webAppUrl}?cb=success` : undefined;
    const cancelUrl = webAppUrl ? `${webAppUrl}?cb=cancel` : undefined;

    const charge = await createCoinbaseCharge({
      name: pack.name || "JIGCOIN Pack",
      description: pack.description || `Pack ${pack_id}`,
      amount: price,
      currency: "GBP",
      metadata: {
        telegram_id: String(telegram_id),
        pack_id: String(pack_id),
        credit_balance: String(credit_balance),
        energy_delta: String(energy_delta),
      },
    ...
    console.error("[coinbase] create-charge error:", e);
    return res.status(500).json({ error: e?.message || "coinbase create-charge failed" });
  }
});

/**
 * Coinbase Commerce webhook endpoint
 * Coinbase will POST events here. We verify the HMAC signature.
 */
app.post("/webhooks/coinbase", async (req, res) => {
  ...
  try {
    ...
    if (eventId) {
      const { rows } = await pool.query(
        "SELECT status FROM public.coinbase_webhook_events WHERE event_id = $1",
        [eventId]
      );
      ...
      await pool.query(
        `INSERT INTO public.coinbase_webhook_events (event_id, event_type, received_at, status)
    ...
    // We only credit on confirmed/resolved
    if (!["charge:confirmed", "charge:resolved"].includes(eventType)) {
      if (eventId) {
        await pool.query(
          "UPDATE public.coinbase_webhook_events SET status='ignored', processed_at=NOW() WHERE event_id=$1",
          [eventId]
        );
      }
      return res.status(200).send("ok");
    ...
    const telegram_id = meta.telegram_id || meta.telegramId;
    const pack_id = meta.pack_id;
    const credit_balance = Number(meta.credit_balance || 0);
    const energy_delta = Number(meta.energy_delta || 0);

    if (!telegram_id) {
      throw new Error("Missing telegram_id in Coinbase metadata");
    }

    // Credit the user
    const { data: user, error: userErr } = await supabaseAdmin
      .from("users")
      .select("id, telegram_id, balance, energy")
      .eq("telegram_id", telegram_id)
      .single();

    if (userErr || !user) {
      throw new Error("User not found for telegram_id");
    }

    const { error: updErr } = await supabaseAdmin
      .from("users")
      .update({
        balance: (user.balance || 0) + credit_balance,
        energy: (user.energy || 0) + energy_delta,
        updated_at: new Date().toISOString(),
      })
      .eq("id", user.id);

    if (updErr) {
      throw new Error("Failed to update user balance/energy");
    }

    // Insert purchases + ledger (best-effort)
    const localPricing = charge.pricing?.local || {};
    const amount = localPricing.amount || null;
    const currency = localPricing.currency || "GBP";

    const provider_tx = charge.id || charge.code || eventId;

    await supabaseAdmin.from("purchases").insert({
      telegram_id: String(telegram_id),
      provider: "coinbase",
      provider_tx_id: provider_tx,
      pack_id: pack_id ? String(pack_id) : null,
    ...
    if (eventId) {
      await pool.query(
        "UPDATE public.coinbase_webhook_events SET status='processed', processed_at=NOW() WHERE event_id=$1",
        [eventId]
      );
    }
    ...
    if (eventId) {
      try {
        await pool.query(
          "UPDATE public.coinbase_webhook_events SET status='error', error_message=$2, processed_at=NOW() WHERE event_id=$1",
          [eventId, String(e?.message || e)]
        );
      } catch {}
    }
    return res.status(200).send("ok"); // don't retry forever
  }
});

// Backwards-compatible alias (if you later use /api/coinbase/webhook)
app.post("/api/coinbase/webhook", (req, res) => res.redirect(307, "/webhooks/coinbase"));
...
app.post("/api/stripe/webhook", async (req, res) => {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const packSku = (session.metadata && session.metadata.sku) ? String(session.metadata.sku) : null;
      const amountMinor = Number(session.amount_total || 0);
      const currency = (session.currency || 'usd').toString().toLowerCase();
      const stripePaymentIntentId = session.payment_intent ? String(session.payment_intent) : null;
      const stripeCheckoutSessionId = session.id ? String(session.id) : null;
      if (telegramId) {
        ...
        // --- Step 4-5: Insert purchases + purchase_ledger for audit/revenue tracking ---
        // Resolve public user id (bigint) if present
        let publicUserId = null;
        try {
          const u = await pool.query('SELECT id FROM public.users WHERE telegram_id = $1 LIMIT 1', [telegramId]);
          publicUserId = u.rows?.[0]?.id ?? null;
        } catch (e) {
          // don't fail webhook if lookup fails
        }

        await recordPurchaseAndLedger({
          publicUserId,
          telegramId,
          kind: 'vip',
          provider: 'stripe',
          ...
          amountMinor,
          currency,
          providerTx: stripePaymentIntentId || stripeCheckoutSessionId,
          stripePaymentIntentId,
          stripeCheckoutSessionId,
          status: 'paid',
          metadata: {
