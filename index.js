...
// Restore SKU mapping. You must create matching entries in public.payment_products for both providers (stripe/coinbase).
function streakRestoreSkuForTierKey(tierKey) {
  return `STREAK_RESTORE_${String(tierKey || "WARMUP").toUpperCase()}`;
}
...
async function fulfillStreakRestoreByTelegramId({ telegramId, sku, provider, providerPaymentId }) {
  ...
  return await withTransaction(async (client) => {
    ...
    const expectedSku = streakRestoreSkuForTierKey(meta.tier_key || "WARMUP");
    if (String(expectedSku).toUpperCase() !== String(sku || "").toUpperCase()) {
      await completeIdempotencyKeyTx(client, { endpoint, requestId, context, responsePayload: { ok: false, reason: "SKU_MISMATCH", expectedSku } });
...
app.post("/api/streak/restore/create-checkout", async (req, res) => {
  try {
    ...
    const tierKey = meta.tier_key || "WARMUP";
    const sku = streakRestoreSkuForTierKey(tierKey);
