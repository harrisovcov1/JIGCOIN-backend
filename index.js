...
const powerups = require("./lib/powerups");
...
function resolveMissionUrl(rawUrl) {
  if (!rawUrl || String(rawUrl).includes("your-content-url-here")) {
    return process.env.OGADS_SMARTLINK_URL || process.env.SPONSOR_SMARTLINK_URL || rawUrl;
  }
  return rawUrl;
}

function cooldownMsFromHours(h) {
  const n = Number(h);
  if (n === 24) return 10 * 60 * 1000;
  return n * 60 * 60 * 1000;
}
...
// Momentum Shield SKU mapping (insurance-style protection)
// - 7D: 1 shield
// - 30D: 5 shields (roughly weekly)
const MOMENTUM_SHIELD_SKU_MAP = {
  MOMENTUM_SHIELD_7D: { days: 7, quantity: 1 },
  MOMENTUM_SHIELD_30D: { days: 30, quantity: 5 },
};

function getMomentumShieldGrantForSku(sku) {
  const key = String(sku || "").toUpperCase();
  if (MOMENTUM_SHIELD_SKU_MAP[key]) return { ...MOMENTUM_SHIELD_SKU_MAP[key], sku: key };
  // Allow future SKUs like MOMENTUM_SHIELD_14D etc.
  const m = key.match(/^MOMENTUM_SHIELD_(\d+)D$/);
  if (!m) return null;
  const days = Math.max(1, Math.min(365, Number(m[1])));
  const quantity = Math.max(1, Math.ceil(days / 7));
  return { sku: key, days, quantity };
}

async function grantMomentumShieldByTelegramId({ telegramId, sku, provider, providerPaymentId }) {
  const tg = String(telegramId);
  const grant = getMomentumShieldGrantForSku(sku);
  if (!grant) return { ok: false, error: "BAD_SKU" };

  return await withTransaction(async (client) => {
    const { rows: urows } = await client.query(
      `SELECT id, telegram_id FROM public.users WHERE telegram_id = ($1)::bigint LIMIT 1`,
      [tg]
    );
    if (!urows.length) return { ok: false, error: "USER_NOT_FOUND" };
    const userId = urows[0].id;

    const expiresAt = new Date(Date.now() + grant.days * 24 * 60 * 60 * 1000);
    const row = await powerups.addPowerup(client, {
      userId,
      powerupCode: "MOMENTUM_SHIELD",
      packSku: grant.sku,
      ...
      expiresAt,
      metadata: { days: grant.days, kind: "momentum" },
    });

    // Audit
    try {
      await client.query(
        `INSERT INTO public.streak_events (user_id, telegram_id, event, meta)
         VALUES ($1, ($2)::bigint, 'momentum_shield_granted', $3::jsonb);`,
        [userId, tg, JSON.stringify({ sku: grant.sku, qty: grant.quantity, days: grant.days, provider: String(provider || ""), provider_payment_id: providerPaymentId || null })]
      );
    } catch (e) {}

    return { ok: true, granted: !!row, sku: grant.sku, quantity: grant.quantity, days: grant.days };
  });
}
...
// ------------ Streak tiers (FOMO / scarcity tuned) ------------
// Unlimited streak, tiered benefits, hard break after 1 missed UTC day.
// Restore is paid (card/crypto) and available for a limited window after break.
const STREAK_RESTORE_WINDOW_HOURS = Number(process.env.STREAK_RESTORE_WINDOW_HOURS || 36);

// Tier thresholds are inclusive minDays.
// Multipliers are applied to earned points (tap yield) which also impacts leaderboard.
// Energy bonus increases the effective max energy cap (extends session length).
const STREAK_TIERS = [
  { key: "WARMUP",  name: "Warm-up",  minDays: 1,  tapMult: 1.00, energyBonus: 0 },
  { key: "MOMENTUM",name: "Momentum", minDays: 3,  tapMult: 1.05, energyBonus: 5 },
  { key: "GROWTH",  name: "Growth",   minDays: 7,  tapMult: 1.10, energyBonus: 10 },
  { key: "POWER",   name: "Power",    minDays: 14, tapMult: 1.18, energyBonus: 20 },
  { key: "ELITE",   name: "Elite",    minDays: 30, tapMult: 1.28, energyBonus: 35 },
  { key: "LEGEND",  name: "Legend",   minDays: 60, tapMult: 1.40, energyBonus: 50 },
];
...
function diffUtcDays(fromDateStr, toDateStr) {
  const a = startOfUtcDay(fromDateStr);
  const b = startOfUtcDay(toDateStr);
...
function getStreakTier(streakCount) {
  ...
  for (const t of STREAK_TIERS) {
    if (n >= t.minDays) chosen = t;
  }
  return chosen || { key: "NONE", name: "None", minDays: 0, tapMult: 1.0, energyBonus: 0 };
}

function getNextTier(streakCount) {
  const n = Math.max(0, Math.floor(Number(streakCount || 0)));
  for (const t of STREAK_TIERS) {
    if (t.minDays > n) return t;
  }
  return null;
}

// ------------ Streak Restore ladder (T1–T6) ------------
// This ladder is priced by the *streak length at the time of the break*.
// We cap at T6 for now so very large streaks map to the highest tier.
function getStreakRestoreTierInfo(streakCountAtBreak) {
  const nRaw = Number(streakCountAtBreak || 0);
  const n = Number.isFinite(nRaw) ? Math.max(0, Math.floor(nRaw)) : 0;
  if (n <= 0) return { tier: null, sku: null, minDays: null, maxDays: null };

  // Tiers match your Coinbase/Stripe products + Supabase pack_sku values
  if (n >= 1 && n <= 3) return { tier: 'T1', sku: 'STREAK_RESTORE_T1', minDays: 1, maxDays: 3 };
  if (n >= 4 && n <= 7) return { tier: 'T2', sku: 'STREAK_RESTORE_T2', minDays: 4, maxDays: 7 };
  if (n >= 8 && n <= 14) return { tier: 'T3', sku: 'STREAK_RESTORE_T3', minDays: 8, maxDays: 14 };
  if (n >= 15 && n <= 30) return { tier: 'T4', sku: 'STREAK_RESTORE_T4', minDays: 15, maxDays: 30 };
  if (n >= 31 && n <= 60) return { tier: 'T5', sku: 'STREAK_RESTORE_T5', minDays: 31, maxDays: 60 };
  // 61+
  return { tier: 'T6', sku: 'STREAK_RESTORE_T6', minDays: 61, maxDays: 120 };
}

function isVipActive(user) {
  try {
    return user && user.vip_until ? new Date(user.vip_until) > new Date() : false;
  } catch (e) {
    return false;
  }
}

function getEffectiveMaxEnergy(user, streakMeta) {
  const base = Number(user.max_energy || 50);
  const vipCap = isVipActive(user) ? Math.max(base, VIP_MAX_ENERGY) : base;
  const bonus = streakMeta && streakMeta.active ? Number(streakMeta.energy_bonus || 0) : 0;
  return vipCap + bonus;
}

function getStreakMetaFromUser(user) {
  const now = new Date();
  const streakCount = Math.max(0, Math.floor(Number(user.streak_count || 0)));

  // Prefer `last_checkin_at` (timestamptz). Fall back to legacy `last_checkin_date` (date).
  let lastCheckinAt = null;
  if (user.last_checkin_at) {
    const d = new Date(user.last_checkin_at);
    if (!isNaN(d)) lastCheckinAt = d;
  }
  if (!lastCheckinAt && user.last_checkin_date) {
    // Interpret date as UTC midnight for day-based logic.
    const d = new Date(`${user.last_checkin_date}T00:00:00Z`);
    if (!isNaN(d)) lastCheckinAt = d;
  }
  const lastCheckinDay = lastCheckinAt ? utcDayString(lastCheckinAt) : null;

  const brokenAt = user.streak_broken_at ? new Date(user.streak_broken_at) : null;
  const broken = brokenAt && !isNaN(brokenAt);
  const restoreWindowMs = STREAK_RESTORE_WINDOW_HOURS * 60 * 60 * 1000;

  const restoreEndsAt = broken ? new Date(brokenAt.getTime() + restoreWindowMs) : null;
  const restoreAvailable = broken && restoreEndsAt && restoreEndsAt > now;

  // If broken and restore window expired, treat as permanently lost.
  const permanentlyLost = broken && (!restoreEndsAt || restoreEndsAt <= now);

  // Streak is active only when not broken.
  const active = !broken;

  const tier = getStreakTier(streakCount);
  const next = getNextTier(streakCount);

  // Restore tier is priced by the streak length at the time the break occurred.
  // In our flow, streak_count stays at the pre-break count until the user starts a new streak.
  const restoreInfo = broken ? getStreakRestoreTierInfo(streakCount) : { tier: null, sku: null, priceMinor: null, rangeLabel: null };

  return {
    streak_count: streakCount,
    last_checkin_at: lastCheckinAt && !isNaN(lastCheckinAt) ? lastCheckinAt.toISOString() : null,
    last_checkin_day: lastCheckinDay,
    broken_at: brokenAt && !isNaN(brokenAt) ? brokenAt.toISOString() : null,
    restore_available: restoreAvailable,
    restore_ends_at: restoreEndsAt ? restoreEndsAt.toISOString() : null,
    restore_tier: restoreInfo.tier,
    restore_pack_sku: restoreInfo.sku,
    restore_price_minor: restoreInfo.priceMinor,
    restore_range_label: restoreInfo.rangeLabel,
    permanently_lost: permanentlyLost,
    active,
    tier_key: tier.key,
    tier_name: tier.name,
    tap_multiplier: Number(tier.tapMult || 1.0),
    leaderboard_multiplier: Number(tier.tapMult || 1.0),
    energy_bonus: Number(tier.energyBonus || 0),
    next_tier_key: next ? next.key : null,
    next_tier_name: next ? next.name : null,
    next_tier_min_days: next ? next.minDays : null,
    next_tier_in_days: next ? Math.max(0, next.minDays - streakCount) : null,
  };
}

// Restore SKU mapping for ladder tiers (T1–T6). You must create matching entries in public.payment_products for both providers.
function streakRestoreSkuForRestoreTier(restoreTier) {
  const t = String(restoreTier || "").toUpperCase();
  if (!t) return null;
  return `STREAK_RESTORE_${t}`;
}

async function fulfillStreakRestoreByTelegramId({ telegramId, sku, provider, providerPaymentId }) {
  const tg = Number(telegramId || 0);
  if (!tg) return { ok: false, error: "NO_TELEGRAM_ID" };

  const endpoint = "/api/streak/restore/fulfill";
  const context = String(provider || "payment");
  const requestId = providerPaymentId ? String(providerPaymentId) : `${context}:${tg}:${String(sku || "")}`;

  return await withTransaction(async (client) => {
    // Lookup user
    const { rows: urows } = await client.query(
      `SELECT * FROM public.users WHERE telegram_id = ($1)::bigint LIMIT 1`,
      [String(tg)]
    );
    if (!urows.length) return { ok: false, error: "USER_NOT_FOUND" };
    const user = urows[0];

    // Idempotency: prevent double-restore from webhook retries
    const idem = await ensureIdempotencyKeyTx(client, {
      userId: user.id,
      endpoint,
      requestId,
      context,
    });
    if (idem.status === "completed") return { ok: true, restored: false, idempotent: true };

    const meta = getStreakMetaFromUser(user);
    if (!meta.restore_available) {
      await completeIdempotencyKeyTx(client, { endpoint, requestId, context, responsePayload: { ok: false, reason: "RESTORE_NOT_AVAILABLE" } });
      return { ok: false, error: "RESTORE_NOT_AVAILABLE" };
    }

    // Validate SKU matches the user's tier at time of restore (prevents underpay)
    const expectedSku = String(meta.restore_pack_sku || "").toUpperCase();
    if (!expectedSku || expectedSku !== String(sku || "").toUpperCase()) {
      await completeIdempotencyKeyTx(client, { endpoint, requestId, context, responsePayload: { ok: false, reason: "SKU_MISMATCH", expectedSku: meta.restore_pack_sku || null } });
      return { ok: false, error: "SKU_MISMATCH", expectedSku };
    }

    // Restore: clear broken flag, keep streak_count, refresh last_checkin_at to now (so bonuses reactivate immediately)
    ...
    // Persist as a power-up audit row (scalable entitlement ledger)
    // Note: restore is effectively consumed immediately upon payment confirmation.
    await powerups.addPowerup(client, {
      userId: user.id,
      powerupCode: "STREAK_RESTORE",
      packSku: String(sku || "").toUpperCase(),
      provider: String(provider || ""),
      providerPaymentId: providerPaymentId ? String(providerPaymentId) : null,
      status: "consumed",
      quantity: 1,
      consumedAt: new Date(),
      metadata: {
        restoreTier: meta.restore_tier || null,
        restoreRangeDays: meta.restore_window_days || null,
      },
    });
    ...
    try {
      await client.query(
        `INSERT INTO public.streak_events (user_id, telegram_id, event, meta) VALUES ($1, $2, 'restore', $3::jsonb);`,
        [user.id, user.telegram_id, JSON.stringify({ sku: String(sku || ""), provider: String(provider || ""), provider_payment_id: providerPaymentId || null })]
      );
    } catch (e) {}
    ...
    await completeIdempotencyKeyTx(client, {
      ...
      requestId,
      context,
      responsePayload: { ok: true, restored: true },
    });

    return { ok: true, restored: true, user: upd && upd[0] ? upd[0] : null };
  });
}
...
// ------------ Shop (Momentum Shield packs) ------------
// Serves active Momentum Shield SKUs (Coinbase rows are the source of pricing).
app.post("/api/shop/momentum-shields", async (req, res) => {
  try {
    const fp = req._fp || getClientFingerprint(req);
    if (!hit(`mshield:${fp}`, 30, 60_000)) {
      return res.status(429).json({ ok: false, error: "RATE_LIMIT" });
    }

    const { rows } = await pool.query(
      `
      SELECT provider, pack_sku, title, description, currency, amount_minor, checkout_url
      FROM public.payment_products
      WHERE provider = 'coinbase'
        AND active = TRUE
        AND pack_sku ILIKE 'MOMENTUM\_SHIELD\_%'
      ORDER BY amount_minor ASC NULLS LAST, pack_sku ASC;
      `
    );
    ...
    const packs = rows.map((r) => ({
      sku: r.pack_sku,
      title: r.title || "Momentum Shield",
      description: r.description || "Protect your streak from one missed day.",
      price_gbp: r.amount_minor == null ? null : Number(r.amount_minor) / 100,
      currency: r.currency || "GBP",
      checkout_url: r.checkout_url,
    }));
    ...
    console.error("Error /api/shop/momentum-shields:", err);
    return res.status(500).json({ ok: false, error: "SHOP_MOMENTUM_SHIELDS_ERROR" });
  }
});
...
app.post("/api/stripe/webhook", async (req, res) => {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (telegramId) {
        // 0) Streak restore (paid card/crypto)
        try {
          if (effectiveSku && String(effectiveSku).toUpperCase().startsWith("STREAK_RESTORE_")) {
            await fulfillStreakRestoreByTelegramId({
              telegramId,
              sku: String(effectiveSku).toUpperCase(),
              provider: "stripe",
              providerPaymentId: String(event.id || session.id || ""),
            });
            // Do not also fulfill coins/VIP for a restore product.
            return res.status(200).send("ok");
          }
        } catch (e) {
          console.error("Stripe streak restore fulfillment failed:", e && e.message ? e.message : e);
          // Continue processing other fulfillment if restore fails (safe fallback).
        }

        // 0.5) Momentum Shield (insurance-style streak protection)
        try {
          if (effectiveSku && String(effectiveSku).toUpperCase().startsWith("MOMENTUM_SHIELD_")) {
            await grantMomentumShieldByTelegramId({
              telegramId,
              sku: String(effectiveSku).toUpperCase(),
              provider: "stripe",
              providerPaymentId: String(event.id || session.id || ""),
            });
            // Shield products do not also credit coins.
            return res.status(200).send("ok");
          }
        } catch (e) {
          console.error("Stripe Momentum Shield fulfillment failed:", e && e.message ? e.message : e);
          // Continue processing other fulfillment as a safe fallback.
        }
  ...
  try {
    ...
    if (type === "charge:confirmed" || type === "charge:resolved") {
      ...
      if (amountMinor != null) {
        ...
        // Streak restore fulfillment (paid crypto)
        try {
          if (packSku && String(packSku).toUpperCase().startsWith("STREAK_RESTORE_")) {
            await fulfillStreakRestoreByTelegramId({
              telegramId: telegramIdRaw,
              sku: String(packSku).toUpperCase(),
              provider: "coinbase",
              providerPaymentId,
            });
            return res.status(200).send("ok");
          ...
          console.error("Coinbase streak restore fulfillment failed:", e && e.message ? e.message : e);
          // Continue processing other fulfillment if restore fails.
        }

        // Momentum Shield (paid crypto)
        try {
          if (packSku && String(packSku).toUpperCase().startsWith("MOMENTUM_SHIELD_")) {
            await grantMomentumShieldByTelegramId({
              telegramId: telegramIdRaw,
              sku: String(packSku).toUpperCase(),
              provider: "coinbase",
          ...
          console.error("Coinbase Momentum Shield fulfillment failed:", e && e.message ? e.message : e);
          // Continue processing other fulfillment if shield grant fails.
        }
...
function buildDataCheckString(initDataRaw) {
...
function verifyTelegramInitData(initDataRaw, botToken) {
  try {
    if (!initDataRaw || !botToken) return { ok: false, error: "missing_initdata_or_token" };
    const params = new URLSearchParams(initDataRaw);
    const hash = params.get("hash");
    if (!hash) return { ok: false, error: "missing_hash" };

    const dataCheckString = buildDataCheckString(initDataRaw);
    ...
    if (!authDate || nowSec - authDate > maxAgeSec) return { ok: false, error: "stale_auth_date" };
...
async function getOrCreateUserFromInitData(req) {
  ...
  const v = verifyTelegramInitData(initDataRaw, botToken);
  if (!v.ok) {
    const e = new Error(v.error || "unauthorized");
    e.status = 401;
    throw e;
  }
  ...
  if (!telegramUserId) {
    const e = new Error("missing_telegram_user");
    e.status = 401;
    throw e;
  }
  ...
  if (!user) {
    const { rows: created } = await pool.query(
      `insert into public.users (telegram_id, username, first_name, created_at)
       values ($1::bigint, $2, $3, now())
       returning *`,
      [telegramUserId, username, firstName],
    );
    user = created[0];
  }
...
async function applyEnergyRegen(user) {
  const streakMeta = getStreakMetaFromUser(user);
  const maxEnergy = getEffectiveMaxEnergy(user, streakMeta);
  const now = new Date();
...
async function ensureDailyReset(user) {
  ...
  let fresh = updated.rows[0];

  // Refill energy to the effective cap (VIP + active streak bonus)
  try {
    const streakMeta = getStreakMetaFromUser(fresh);
    const cap = getEffectiveMaxEnergy(fresh, streakMeta);
    ...
    if (r2.rows && r2.rows[0]) fresh = r2.rows[0];
  } catch (e) {}
...
async function handleTap(user) {
  ...
  // Streak multiplier (only when streak is active; broken streak yields no bonus)
  try {
    const streakMeta = getStreakMetaFromUser(user);
    if (streakMeta && streakMeta.active && Number(streakMeta.tap_multiplier || 1) > 1) {
      perTap = perTap * Number(streakMeta.tap_multiplier || 1);
    }
  } catch (e) {}

  const delta = Math.max(1, Math.floor(perTap));
...
async function buildClientState(user) {
  ...
  // Active / available power-ups (for UI gating and diagnostics)
  // NOTE: this call is defensive and returns [] on any query/shape mismatch.
  const active_powerups = await powerups.getActivePowerupsForUser(pool, user.id);

  // --- Streak break detection on load ---
  // If the user missed a full day, mark the streak as broken immediately so the UI can
  // offer a restore / protection without requiring an extra user action.
  //
  // Rule of thumb:
  // - last_checkin = today      => not broken
  // - last_checkin = yesterday  => not broken (user can check in today)
  // - last_checkin <= two days ago => broken (user missed yesterday)
  try {
    const hasMomentumShield = Array.isArray(active_powerups)
      && active_powerups.some((p) => String(p.powerup_code) === "MOMENTUM_SHIELD" && String(p.status) === "active");

    const now = new Date();
    const today = utcDayString(now);
    const yesterday = utcDayString(new Date(now.getTime() - 24 * 60 * 60 * 1000));
    const lastDay = user.last_checkin_at ? utcDayString(new Date(user.last_checkin_at)) : null;

    const shouldBreak = Boolean(
      user.streak_count && Number(user.streak_count) > 0
      && !user.streak_broken_at
      && lastDay
      && lastDay !== today
      && lastDay !== yesterday
    );

    if (shouldBreak && !hasMomentumShield) {
      await pool.query(
        `update public.users set streak_broken_at = now() where id = $1 and streak_broken_at is null`,
        [user.id]
      );
      // keep local copy consistent for this response
      user.streak_broken_at = new Date().toISOString();
    }
  } catch (e) {
    // never fail /api/state because of streak break detection
  }

  // Streak meta + effective caps (server source of truth)
  const streak_meta = getStreakMetaFromUser(user);
  const max_energy_effective = getEffectiveMaxEnergy(user, streak_meta);

  // Convenience fields for UI (Momentum Shield)
  let momentum_shield_qty = 0;
  let momentum_shield_expires_at = null;
  try {
    const shields = active_powerups.filter((p) => String(p.powerup_code) === "MOMENTUM_SHIELD");
    if (shields.length) {
      momentum_shield_qty = shields.reduce((acc, p) => acc + Math.max(0, Number(p.quantity || 0)), 0);
      // earliest expiry (or null if any is non-expiring)
      const expiries = shields
        .map((p) => (p.expires_at ? new Date(p.expires_at).getTime() : null))
        .filter((t) => t != null && Number.isFinite(t));
      if (expiries.length) momentum_shield_expires_at = new Date(Math.min(...expiries)).toISOString();
    }
  } catch (e) {}
  ...
  return {
    ...
    max_energy_effective: Number(max_energy_effective || (user.max_energy || 50)),
    starterOfferAvailable,
    starterOfferExpiresAt,
    active_powerups,
    momentum_shield_qty,
    momentum_shield_expires_at,
...
async function applyGenericReward(user, rewardType, rewardAmount) {
  ...
  if (type === "points" && amount > 0) {
    ...
    const streakMeta = getStreakMetaFromUser(user);
    const cap = getEffectiveMaxEnergy(user, streakMeta);
    const res = await pool.query(
      `
      UPDATE public.users
    ...
    updatedUser = res.rows[0];
  } else if (type === "mission") {} else if (type === "mission") {
...
app.post("/api/state", async (req, res) => {
  try {
    ...
    // Anti-bot timing check (server-side). If taps arrive faster than plausible,
    // accumulate a fraud_score and temporarily ban at threshold.
    try {
      const lastTap = user.last_tap_at ? new Date(user.last_tap_at) : null;
      const nowTs = new Date();
      if (lastTap && nowTs - lastTap < 80) {
        const updFraud = await pool.query(
          `UPDATE public.users
           SET fraud_score = COALESCE(fraud_score,0) + 1,
               last_tap_at = NOW(),
               banned_until = CASE WHEN COALESCE(fraud_score,0) + 1 >= 35 THEN NOW() + INTERVAL '30 minutes' ELSE banned_until END
           WHERE id = $1
           RETURNING fraud_score, banned_until`,
          [user.id]
        );
        user.fraud_score = updFraud.rows[0]?.fraud_score;
        user.banned_until = updFraud.rows[0]?.banned_until;
        if (user.banned_until && new Date(user.banned_until) > new Date()) {
          const state = await buildClientState(user);
          return res.status(403).json({ ...state, ok:false, reason:"BANNED" });
        }
      } else {
        await pool.query(`UPDATE public.users SET last_tap_at = NOW() WHERE id = $1`, [user.id]);
      }
    } catch (e) {
      // non-fatal
    }
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    // Per-user burst limiter (separate from fingerprint)
    if (!hit(`tapburst:u:${user.telegram_id}`, 12, 1000)) {
    ...
    // Streak multiplier (only when streak is active)
    try {
      const streakMeta = getStreakMetaFromUser(user);
      if (streakMeta && streakMeta.active && Number(streakMeta.tap_multiplier || 1) > 1) {
        perTap = perTap * Number(streakMeta.tap_multiplier || 1);
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    if (!hit(`tapburst:u:${user.telegram_id}`, 6, 1000)) {
      return res.status(429).json({ ok: false, error: "TAP_RATE_LIMIT" });
    }
    if (user.banned_until && new Date(user.banned_until) > new Date()) {
      const state = await buildClientState(user);
    ...
    // Anti-bot packet timing check
    try {
      const lastTap = user.last_tap_at ? new Date(user.last_tap_at) : null;
      const nowTs = new Date();
      if (lastTap && nowTs - lastTap < 250) {
        const updFraud = await pool.query(
          `UPDATE public.users
           SET fraud_score = COALESCE(fraud_score,0) + 1,
               last_tap_at = NOW(),
    ...
    const gained = Math.max(1, Math.floor(n * perTap));
...
app.post("/api/boost/energy", async (req, res) => {
  try {
    ...
    let method = rawMethod === "points" ? "points" : rawMethod === "sponsor" || rawMethod === "action" ? "sponsor" : "";

    // Default to points for simple one-tap UX (safe fallback)
  ...
  try {
    ...
    const missions = missionsRes.rows.map((m) => {
      ...
      if (!um) {
        ...
        const cooldownMs = cooldownMsFromHours(m.cooldown_hours);
      ...
      return {
        ...
        url: appendOgadsTracking(resolveMissionUrl(m.url), user.id, m.code, sha256Hex(req._fp || '')),
        kind: m.kind,
        status,
...
app.post("/api/mission/start", async (req, res) => {
  try {
    ...
    if (umExisting.rowCount > 0 && umExisting.rows[0].claimed_at) {
      ...
      const cooldownMs = cooldownMsFromHours(mission.cooldown_hours);
      const readyAt = new Date(claimedAt.getTime() + cooldownMs);
      if (readyAt > new Date()) {
    ...
    res.json({ ok: true, code: mission.code, redirect_url: appendOgadsTracking(resolveMissionUrl(mission.url), user.id, mission.code, sha256Hex(req._fp || '')), min_seconds_to_claim: Number(mission.min_seconds_to_claim || 30) });
  } catch (err) {
...
// Daily task route – DAILY CHECK-IN (UTC day, streak break + restore flow)
app.post("/api/task", async (req, res) => {
  try {
    ...
    const action = String(req.body.action || "").trim().toLowerCase(); // "", "start_new"

    // Compute current streak meta from user row
    let meta = getStreakMetaFromUser(user);

    // If a broken streak has expired, clear it (permanently lost)
    if (meta.permanently_lost) {
      try {
        ...
        if (cleared.rows && cleared.rows[0]) {
          user = cleared.rows[0];
          meta = getStreakMetaFromUser(user);
        }
      } catch (e) {}
    }

    const todayStr = utcDayString(new Date());
    const lastDay = meta.last_checkin_day;

    // If the user already claimed today, block (true daily)
    if (lastDay === todayStr) {
      const nextAt = startOfUtcDay(utcDayString(new Date(Date.now() + 24*60*60*1000))).toISOString();
      const state = await buildClientState(user);
      return res.json({
        ...state,
        ok: false,
    ...
    // If the user currently has a broken streak within the restore window,
    // do not auto-reset: force a decision (restore paid OR start new streak).
    if (meta.restore_available && action !== "start_new") {
      const state = await buildClientState(user);
      return res.json({
        ...state,
        ok: false,
        reason: "STREAK_BROKEN",
        streak_restore_available: meta.restore_available,
        error: "Streak is broken. Tap daily check-in to start a new streak, or restore in the shop.",
        streak_restore_ends_at: meta.restore_ends_at,
      });
    }

    // Determine what happens when claiming today's check-in.
    // - If there was no previous check-in: start at 1.
    // - If last check-in was yesterday: increment.
    // - If missed 1+ days: if action=start_new -> start at 1; otherwise mark break + force decision above.
    let newStreak = 1;
    let missed = false;
    let momentumShieldToConsumeId = null;

    if (lastDay) {
      const diff = diffUtcDays(lastDay, todayStr);
      if (diff === 1) {
        newStreak = Math.max(1, Math.floor(Number(user.streak_count || 0))) + 1;
      } else if (diff > 1) {
        missed = true;
        newStreak = 1;

        // Momentum Shield: forgive exactly one missed day (diff==2)
        if (diff === 2) {
          try {
            const { rows: shieldRows } = await pool.query(
              `
              SELECT id, quantity
              FROM public.user_powerups
              WHERE user_id = $1
                AND powerup_code = 'MOMENTUM_SHIELD'
                AND status = 'active'
            ...
            if (shieldRows && shieldRows[0]) {
              // treat as if the user checked in yesterday
              missed = false;
              momentumShieldToConsumeId = shieldRows[0].id;
              newStreak = Math.max(1, Math.floor(Number(user.streak_count || 0))) + 1;
            }
          } catch (e) {}
        }
    ...
    // If missed and not already marked broken, mark break (starts restore window).
    if (missed && !meta.broken_at) {
      try {
        ...
        if (brokenRes.rows && brokenRes.rows[0]) {
          user = brokenRes.rows[0];
          meta = getStreakMetaFromUser(user);
        }
        ...
        try {
          await pool.query(
            `INSERT INTO public.streak_events (user_id, telegram_id, event, meta) VALUES ($1, $2, 'break', $3::jsonb);`,
            [user.id, user.telegram_id, JSON.stringify({ prior_streak: Number(user.streak_count || 0), broke_on: todayStr })]
          );
        } catch (e) {}
    ...
    // If missed and action is not start_new, force decision (restore OR start new).
    if (missed && action !== "start_new") {
      const state = await buildClientState(user);
      return res.json({
        ...state,
        ok: false,
    ...
    // Claim today's daily reward (idempotent per UTC day)
    const endpoint = "/api/task/daily";
    const context = "daily";
    const requestId = `${user.id}:${todayStr}`;

    // Reward: base + milestone bonus (milestones based on the NEW streak)
    let bonus = 0;
    ...
    await withTransaction(async (client) => {
      ...
      // If a Momentum Shield was used to forgive a missed day, consume 1 unit now.
      if (momentumShieldToConsumeId) {
        try {
          await powerups.consumePowerupUnitById(client, momentumShieldToConsumeId);
          await client.query(
            `INSERT INTO public.streak_events (user_id, telegram_id, event, meta)
             VALUES ($1, $2, 'momentum_shield_used', $3::jsonb);`,
            [user.id, user.telegram_id, JSON.stringify({ on_day: todayStr, new_streak: newStreak })]
          );
        } catch (e) {}
      }

      // If user chose to start a new streak after break, clear broken_at
      const clearBrokenAt = missed || meta.restore_available;
      ...
      await client.query(
        ...
        SET streak_count = $1,
            ...
            last_daily = $2,
            last_checkin_date = $2,
            streak_broken_at = CASE WHEN $3 THEN NULL ELSE streak_broken_at END
        WHERE id = $4;
        `,
        [newStreak, todayStr, clearBrokenAt, user.id]
      );

      // Audit event
      try {
        await client.query(
          `INSERT INTO public.streak_events (user_id, telegram_id, event, meta) VALUES ($1, $2, 'checkin', $3::jsonb);`,
          [user.id, user.telegram_id, JSON.stringify({ streak: newStreak, bonus })]
        );
      } catch (e) {}
    ...
    // Reload user
    const upd = await pool.query(`SELECT * FROM public.users WHERE id = $1`, [user.id]);
    user = upd.rows[0];
    meta = getStreakMetaFromUser(user);

    // Starter Bundle onboarding: start a 7-day window the first time a user successfully claims the daily bonus.
    if (!user.starter_offer_purchased && !user.starter_offer_expires_at) {
      ...
      if (starterRows && starterRows[0]) user = starterRows[0];
    }
...
// Streak restore checkout (paid card/crypto only; 36h window)
app.post("/api/streak/restore/create-checkout", async (req, res) => {
  try {
    const user = await getOrCreateUserFromInitData(req);
    const meta = getStreakMetaFromUser(user);

    if (!meta.restore_available) {
      return res.status(400).json({ ok: false, error: "RESTORE_NOT_AVAILABLE" });
    }

    const sku = meta.restore_pack_sku;
    const restoreTier = meta.restore_tier;
    if (!sku) {
      return res.status(400).json({ ok: false, error: "NO_RESTORE_SKU" });
    }

    const r = await pool.query(
      `
      SELECT provider, pack_sku, currency, amount_minor, checkout_url
      FROM public.payment_products
      WHERE pack_sku = $1
      LIMIT 50;
      `,
    ...
    const rows = r.rows || [];
    const coinbaseProduct = rows.find((x) => String(x.provider || "").toLowerCase() === "coinbase");
    const stripeProduct = rows.find((x) => String(x.provider || "").toLowerCase() === "stripe");

    if ((!coinbaseProduct || !coinbaseProduct.checkout_url) && (!stripeProduct || !stripeProduct.checkout_url)) {
      return res.status(404).json({ ok: false, error: "RESTORE_PRODUCT_MISSING", sku });
    }

    // Reuse the existing unified checkout endpoint for Stripe session creation when possible.
    // The frontend may open either link; Stripe requires a session URL, Coinbase uses checkout_url directly.
    // We return both.
    return res.json({
      ok: true,
      sku,
      restore_tier: restoreTier,
      restore_ends_at: meta.restore_ends_at,
      coinbaseCheckoutUrl: coinbaseProduct ? coinbaseProduct.checkout_url : null,
      stripeCheckoutUrl: stripeProduct ? stripeProduct.checkout_url : null,
    });
  } catch (err) {
    console.error("Error /api/streak/restore/create-checkout:", err);
    res.status(500).json({ ok: false, error: "RESTORE_CHECKOUT_ERROR" });
  }
});
