...
async function applyEnergyRegen(user) {
  ...
  // No regeneration while the user is actively tapping.
  if (user.last_tap_ts) {
    const lt = new Date(user.last_tap_ts);
    const sinceTap = now - lt;
    if (Number.isFinite(sinceTap) && sinceTap >= 0 && sinceTap < 1500) {
      return user;
    }
  }
  ...
  if (energy >= maxEnergy) return user;

  let consumedMs = 0;

  while (diffMs - consumedMs > 0 && energy < maxEnergy) {
    let stepMs;
    if (energy < 10) stepMs = 250;
    else if (energy < 20) stepMs = 800;
    else if (energy < 30) stepMs = 1200;
    else stepMs = 1800;

    if (diffMs - consumedMs >= stepMs) {
      energy += 1;
      consumedMs += stepMs;
    } else break;
  }
