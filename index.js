...
async function getOrCreateUserFromInitData(req) {
  const headerInit = (req.headers && (req.headers['x-telegram-init-data'] || req.headers['x-telegram-webapp-initdata'] || req.headers['telegram-init-data'] || req.headers['authorization'])) || "";
  const bearer = typeof headerInit === 'string' && headerInit.toLowerCase().startsWith('bearer ') ? headerInit.slice(7) : headerInit;
  const initDataRaw = req.body.initData || req.query.initData || bearer || "";
...
async function applyEnergyRegen(user, opts = {}) {
  const maxEnergy = Number(user.max_energy || 50);
  const now = new Date();

  // If caller says "tapping", do not regen here (prevents energy increasing mid-session).
  if (opts && opts.skipIfTapping) return user;

  // If never had regen timestamp → assume long offline, but we still cap per call (no giant jumps).
  let lastTs = user.last_energy_ts ? new Date(user.last_energy_ts) : new Date(now.getTime() - 60 * 60 * 1000);
  if (isNaN(lastTs)) lastTs = new Date(now.getTime() - 60 * 60 * 1000);

  let diffSeconds = Math.floor((now - lastTs) / 1000);
  if (diffSeconds <= 0) return user;

  let energy = Number(user.energy || 0);
  if (!Number.isFinite(energy)) energy = 0;

  // Deterministic curve (your spec):
  // 0–10 fast, slows as it approaches 50. We do 1s/3s/6s steps, capped per call so it never "jumps" huge.
  const MAX_GAIN_PER_CALL = 6; // hard cap so /state can't jump 30+ energy in one response
  let gained = 0;
  let secondsUsed = 0;
  ...
  while (diffSeconds > 0 && energy < maxEnergy && gained < MAX_GAIN_PER_CALL) {
    ...
    else step = 6;                  // slow regen (towards 50)
  ...
  // Advance regen timestamp only by the time actually used, so remaining time carries forward.
  const newLast = new Date(lastTs.getTime() + secondsUsed * 1000);
  ...
  if (gained > 0) {
    await pool.query(
      ...
      WHERE id = $3
      `,
      [energy, newLast, user.id]
    );
    user.energy = energy;
    user.last_energy_ts = newLast;
  } else {
    // no change
    user.last_energy_ts = lastTs;
  }
...
app.post("/api/state", async (req, res) => {
  try {
    ...
    const isTapping = !!(req.body && req.body.client && req.body.client.is_tapping);
    user = await applyEnergyRegen(user, { skipIfTapping: isTapping });
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    maybeActivateReferral(updatedUser, 'tap');
    const state = await buildClientState(updatedUser);
    return res.json({ ...state, ok: true, applied: n, gained, seq });
  } catch (err) {
