...
// VIP defaults (used if DB rows are missing/misconfigured)
const VIP_DEFAULT_PRICE_GBP = Number(process.env.VIP_DEFAULT_PRICE_GBP || 15);
const VIP_DEFAULT_CHECKOUT_URL = String(
  process.env.VIP_DEFAULT_CHECKOUT_URL ||
    "https://commerce.coinbase.com/checkout/ea139266-01fb-4c97-a8da-bf11c27ec684"
).trim();
...
app.post("/api/vip/product", async (req, res) => {
  try {
    ...
    if (ppRows.length) {
      const p = ppRows[0];
      const priceGbp = VIP_DEFAULT_PRICE_GBP;
      const checkoutUrl = p.checkout_url || VIP_DEFAULT_CHECKOUT_URL;
      return res.json({
        ok: true,
        product: {
          sku: p.pack_sku,
          ...
          duration_days: 30,
          // Display price is authoritative for UX; keep it stable at £15 unless you explicitly change env.
          price_gbp: Number.isFinite(priceGbp) ? priceGbp : (p.amount_minor == null ? null : Number(p.amount_minor) / 100),
          // Keep legacy field name used by the current frontend
    ...
    if (!rows.length) {
      // Final fallback: make VIP purchasable even if DB is not seeded correctly.
      return res.json({
        ok: true,
        product: {
          sku: 'vip_30d',
          title: 'JigCoin VIP Pass (30 days)',
          duration_days: 30,
          price_gbp: VIP_DEFAULT_PRICE_GBP,
          stripe_payment_link: VIP_DEFAULT_CHECKOUT_URL,
          checkout_url: VIP_DEFAULT_CHECKOUT_URL,
          currency: 'GBP',
          active: true,
        },
      });
    ...
    const p = rows[0];
    const legacyPrice = p.price_gbp == null ? null : Number(p.price_gbp);
    const stablePrice = Number.isFinite(VIP_DEFAULT_PRICE_GBP) ? VIP_DEFAULT_PRICE_GBP : legacyPrice;
    return res.json({
      ok: true,
      product: {
        sku: p.sku,
        ...
        price_gbp: stablePrice,
        stripe_payment_link: p.stripe_payment_link || VIP_DEFAULT_CHECKOUT_URL,
        checkout_url: p.stripe_payment_link || VIP_DEFAULT_CHECKOUT_URL,
        currency: 'GBP',
        active: !!p.active,
      },
    });
...
bot.start(async (ctx) => {
  try {
    ...
    await ctx.reply(
      "🔥 Welcome to JigCoin — Early Access\n\nInvite users, climb rank, and earn promoter rewards.\nTap below to open the app 👇",
      {
        reply_markup: {
          inline_keyboard: [
            [
