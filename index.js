...
app.post("/api/state", async (req, res) => {
  try {
    ...
    // IMPORTANT: Do NOT regen energy inside tap endpoints.
    // Regen is applied via /api/state so energy never "refills while tapping",
    // which keeps the drain/earn loop consistent and prevents perceived missed taps.
...
app.post("/api/tap", async (req, res) => {
  try {
    ...
    // 1) Do NOT regen energy inside tap endpoints.
    // Regen is applied via /api/state only. This prevents energy refilling while
    // the user is actively tapping and keeps every tap feeling consistent.
...
app.post("/api/tapPacket", async (req, res) => {
  try {
    ...
    // Do NOT regen energy inside tap endpoints. Regen is applied via /api/state.
    user = await ensureDailyReset(user);
